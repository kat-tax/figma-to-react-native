...
export type StyleSheetWithSuperPowers<S extends StyleSheet> = ((theme: UnistylesTheme, miniRuntime: UnistylesMiniRuntime) => S) | S;
type UseVariantsFn<T> = {
    useVariants: (variants: ExtractVariantNames<T>) => void;
};
declare const create: <S extends StyleSheet>(stylesheet: StyleSheetWithSuperPowers<S>) => (ReactNativeStyleSheet<S> & UseVariantsFn<typeof stylesheet>);
