...
export function updateMenuElementLayout(
  ...
  const left = computeLeft({menuWidth, rootLeft});

  menuElement.style.left = `${left}px`;

  const top = computeTop({menuHeight, rootTop, selectedTop: labelElement.offsetTop});
...
function getSelectedLabelElement(menuElement: HTMLDivElement, selectedId: Id): HTMLLabelElement {
...
function computeTop(options: {menuHeight: number, rootTop: number, selectedTop: number}): number {
